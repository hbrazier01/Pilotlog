...
import { computePplRequirements, computePplPart61Progress, computePplEngine } from "./lib/faa/pplEngine.mjs";
...
// FAA Part 61 ASEL requirements — raw engine output (backward compat)
...
// Full PPL engine output — complete phase, readiness, solo eligibility, checkride readiness
app.get("/api/ppl-engine", (req, res) => {
  const asOf = String(req.query.asOf || new Date().toISOString());
  const entries = readEntries();
  let profile = null;
  try {
  ...
  const result = computePplEngine(entries, { asOf, profile });
  res.json(result);
});
